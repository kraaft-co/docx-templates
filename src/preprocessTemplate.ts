...
const preprocessTemplate = (
  ...
  while (node != null) {
    ...
    // `openNode` is also truthy while a *speculative* match of the delimiter's start is still
    // pending (idxDelimiter > 0, fCmd still false) -- i.e. we haven't actually confirmed we're
    // inside a command yet. Appending a space in that case corrupts plain text: if the tentative
    // match later fails (the common case, since it's just a coincidental prefix match), the
    // held-back character(s) get flushed back *after* this space, producing extra spaces in the
    // final text. Only inject the paragraph-break space once we're not mid-match.
    if (
      !node._fTextNode &&
      node._tag === 'w:p' &&
      openNode &&
